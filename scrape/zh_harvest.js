...
const SF = fs.readFileSync('Z:/Files/Aquapedia/data/raw/sf.jsonl', 'utf8')
  .trim().split('\n').filter(Boolean).map(l => JSON.parse(l))
  .map(r => ({ sci: (r.scientific_name || '').trim(), ens: r.common_names || [] }))
  .filter(x => x.sci);

const SHARD = 'a';
const basePath = 'Z:/Files/Aquapedia/data/raw/zh.jsonl';
const zhPath = 'Z:/Files/Aquapedia/data/raw/zh_' + SHARD + '.jsonl';
const done = new Set(
