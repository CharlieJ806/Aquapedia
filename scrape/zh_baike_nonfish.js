// 百度百科 非鱼生物正文收割器 v1 — 在 browser run 作用域内 new Function 执行（page/tab/require 可用）
// 抓水草/珊瑚/无脊椎(虾蟹螺等)的中文正文作主内容。断点：zh_baike_nonfish.jsonl 已记录的 sci 跳过。
const fs = require('fs');

const NAMED = fs.readFileSync('Z:/Files/Aquapedia/data/raw/zh_inat_nonfish.jsonl', 'utf8')
  .trim().split('\n').filter(Boolean).map((l) => JSON.parse(l)).filter((r) => r.name_zh);
const outPath = 'Z:/Files/Aquapedia/data/raw/zh_baike_nonfish.jsonl';
const done = new Set(
...
if (!onBaike) {
  await tab.goto('https://baike.baidu.com/item/水草', { waitUntil: 'domcontentloaded', timeout: 40000 }).catch(() => {});
